  ...
  <div>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="消毒人员" prop="disinfectionUserId">
            <Select
              filterable
              v-model="formModel.fields.disinfectionUserId"
              style="width: 100%;"
              :disabled="checkShow()"
              :label-in-value="true"
              @on-change="GetdisinUser"
            >
              <Option
        ...
        <Row :gutter="16">
          <FormItem label="消毒餐次" prop="xdccType">
            <Select
              filterable
              v-model="formModel.fields.xdccType"
              style="width: 100%;"
              :disabled="checkShow()"
              :label-in-value="true"
              @on-change="GetxdccTypelist"
            >
              <Option
                v-for="item in xdccTypelist"
                :key="item.typeId"
                :value="item.typeId"
        ...
        <Row :gutter="16">
          <FormItem label="消毒方式" prop="method">
            <Input
              v-model="formModel.fields.method"
              type="textarea"
              :readonly="true"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="消毒对象" prop="disinfection">
            <Input
              v-model="formModel.fields.disinfection"
              type="textarea"
              :readonly="true"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          ...
          <Upload
            ...
            :format="['jpg','jpeg','png']"
            :max-size="5120"
            :data="{scene:'消毒封面图',groupType:'disinfect'}"
        ...
        <Row :gutter="16">
          ...
          <Upload
            ...
            :format="['jpg','jpeg','png']"
            :max-size="5120"
            :data="{scene:'消毒图片',groupType:'disinfect'}"
            :on-format-error="handleFormatError"
            :on-exceeded-size="handleMaxSize"
            :before-upload="handleBeforeUpload"
            type="drag"
          ...
          <Modal title="查看图片" v-model="visible2">
            <img :src="imgName2" v-if="visible2" style="width: 100%" />
          </Modal>
          </FormItem>
        </Row>
      </Form>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="消毒对象" prop="dxType">
            <Select
              filterable
              v-model="formModel1.fields.dxType"
              style="width: 100%;"
              :disabled="checkShow2()"
              :label-in-value="true"
              @on-change="GetdxTypelist"
            >
              <Option
                v-for="item in dxTypelist"
                :key="item.typeId"
                :value="item.typeId"
              >{{ item.keyValue }}</Option>
            </Select>
        ...
        <Row :gutter="16">
          <FormItem label="消毒方式" prop="methodId">
            <Select
              filterable
              v-model="formModel1.fields.methodId"
              style="width: 100%;"
              :disabled="checkShow2()"
              :label-in-value="true"
              @on-change="GetmethodIdlist"
            >
              <Option
                v-for="item in methodIdlist"
                :key="item.typeId"
                :value="item.typeId"
              >{{ item.keyValue }}</Option>
            </Select>
        ...
        <Row :gutter="16">
          <FormItem label="消毒数量" prop="number">
            <Input-number
        ...
        <Row :gutter="16">
          <FormItem label="消毒间" prop="roomId">
            <Select
              filterable
              v-model="formModel1.fields.roomId"
              style="width: 100%;"
              :disabled="checkShow2()"
              :label-in-value="true"
              @on-change="GetroomIdlist"
            >
              <Option
        ...
        <Row :gutter="16">
          <FormItem label="消毒工具" prop="toolId">
            <Select
              filterable
              v-model="formModel1.fields.toolId"
              style="width: 100%;"
              :disabled="checkShow2()"
              :label-in-value="true"
              @on-change="GettoolIdlist"
            >
              <Option
                v-for="item in toolIdlist"
                :key="item.lightId"
                :value="item.lightId"
              >{{ item.name }}</Option>
            </Select>
...
import {
  ...
  aGetEdit, //编辑
  GetmethodList, //消毒方法下拉框
  GetdisinfectionList, //消毒对象下拉框
  GetccList, //消毒餐次下拉框
  GetroomList, //消毒间下拉框
  GettoolList, //消毒工具下拉框
  GetuserList, //该组织下人员列表
  GetUsersList
} from "@/api/PurchasesInfo/Disinfection";
...
export default {
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          columns: [
            ...
            { title: "消毒人员", key: "disinfectionUserName", sortable: true, ellipsis: true },
            { title: "消毒餐次", key: "typeName", sortable: true, ellipsis: true },
            { title: "消毒方式", key: "method", sortable: true, ellipsis: true },
            { title: "消毒对象", key: "disinfection", sortable: true, ellipsis: true },
            { title: "创建日期", key: "createdDate", sortable: true, ellipsis: true },
            {
              title: "操作",
              align: "center",
              width: 200,
        ...
        mCheckInfo: {
          ...
          columns: [
            { type: "selection", width: 50, key: "id" },
            { title: "消毒对象", key: "typeName", sortable: true, ellipsis: true },
            { title: "消毒方式", key: "method", sortable: true, ellipsis: true },
            { title: "消毒数量/时长", key: "number", sortable: true, ellipsis: true },
            { title: "消毒间", key: "roomName", sortable: true, ellipsis: true },
            { title: "工具", key: "toolName", sortable: true, ellipsis: true },
            {
              title: "操作",
              align: "center",
              width: 200,
      ...
      formModel: {
        ...
        rules: {
          ...
          disinfectionUserId: [
            {  required: true, message: "消毒人员不能为空" }
          ],
          xdccType: [{  required: true, message: "请输入消毒餐次" }]
        },
      },
      formModel1: {
        opened: false,
  ...
  methods: {
    ...
    doGetmethodList() {
      GetmethodList().then((res) => {
        // console.log("消毒方法下拉框");
        // console.log(res);
        this.methodIdlist = res.data.data;
      });
    },
    //消毒对象下拉框
    doGetdisinfectionList() {
      GetdisinfectionList().then((res) => {
        // console.log("消毒对象下拉框");
        // console.log(res);
        this.dxTypelist = res.data.data;
      });
    },
    //消毒餐次下拉框
    doGetccList() {
      GetccList().then((res) => {
        // console.log("消毒餐次下拉框");
        // console.log(res);
        this.xdccTypelist = res.data.data;
      });
    },
    //消毒间下拉框
    doGetroomList() {
    ...
    doGetGettoolList() {
      GettoolList().then((res) => {
        // console.log("消毒工具下拉框");
        // console.log(res);
        this.toolIdlist = res.data.data;
      });
    },
    GetdisinUser(e) {
      if (e != undefined) {
        this.formModel.fields.disinfectionUserName = e.label;
      }
    },
    GetxdccTypelist(e) {
      if (e != undefined) {
        this.formModel.fields.xdccTypeName = e.label;
      }
    },
    GetdxTypelist(e) {
      if (e != undefined) {
        this.formModel1.fields.dxTypeName = e.label;
      }
    },
    GetmethodIdlist(e) {
      if (e != undefined) {
        this.formModel1.fields.method = e.label;
      }
    },
    GetroomIdlist(e) {
      if (e != undefined) {
        this.formModel1.fields.roomName = e.label;
      }
    },
    GettoolIdlist(e) {
      if (e != undefined) {
        this.formModel1.fields.toolName = e.label;
      }
    },
    //详情显示
    handleDetail(e) {
    ...
    doLoadData(id) {
      GetfoGet(id).then((res) => {
        ...
        if (res.data.code === 200) {
          ...
          this.formModel.fields.xdccType=res.data.data.type;
          this.formModel.fields.xdccTypeName=res.data.data.typeName;
          ...
          if (res.data.data.imgUrls != null) {
            let list = res.data.data.imgUrls.split(",");
    ...
    doLoadData2(id) {
      console.log(id)
      this.formModel1.fields.dId=id;
      aGetShow(id).then((res) => {
        console.log(res);
        if (res.data.code === 200) {
          this.formModel1.fields = res.data.data;
          this.formModel1.fields.dxType=res.data.data.type;
          this.formModel1.fields.dxTypeName=res.data.data.typeName;
        }
      });
    },
    validateUserForm() {
    ...
    handleSubmitConsumable() {
      ...
      if (valid) {
        if(this.formModel.fields.img.split(',').length>2){
          this.$Message.warning("请上传一张封面图片");
          return;
        }
        if (this.formModel.mode === "create") {
    ...
    //编辑（保存）
    doEditDispatch2() {
      this.formModel1.fields.dId=this.stores.mCheckInfo.query.kw;
    ...
    handleView2(name) {
      this.imgName2 = name;
      this.visible2 = true;
      console.log(this.imgName2);
    },
    handleRemove2(file) {
      console.log(file);
          this.uploadList2 = this.uploadList2.filter(x => x.name != file);
          this.formModel.fields.imgUrls = this.uploadList2
            .map(x => x.fileName)
            .join(",");
    },
    toUpResult2() {
    ...
    async showUpResult2(response, file, fileList) {
      ...
      this.updisabled2 = false;
      if (response.code == "200") {
        this.$Message.success(response.message);
        console.log(this.formModel.fields.imgUrls);
        if (this.formModel.fields.imgUrls != null) {
          if (this.formModel.fields.imgUrls.length > 0) {
            this.formModel.fields.imgUrls += ",";
          }
          this.formModel.fields.imgUrls += response.data;
        } else {
          this.formModel.fields.imgUrls = response.data;
        }
  ...
  mounted() {
    ...
    this.doGetUsersList();
    this.doGetuserList(); //消毒人员下拉框
    this.doGetmethodList(); //消毒方法下拉框
    this.doGetdisinfectionList(); //消毒对象下拉框
    this.doGetccList(); //消毒餐次下拉框
    this.doGetroomList(); //消毒间下拉框
    this.doGetGettoolList(); //消毒工具下拉框
  },
  created() {
    this.postheaders = {
