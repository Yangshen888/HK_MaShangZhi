...
//WHERE P.IsDeleted=0 AND P.Status=1 AND P.Type=0 AND M.IsDeleted=0 AND M.Status=1 AND EXISTS (SELECT 1 FROM SystemUserRoleMapping AS URM WHERE URM.SystemUserUUID={0} AND URM.SystemRoleUUID=RPM.SystemRoleUUID)";
            ...
            {
                menus = _dbContext.ViewMenu.Where(x => x.Pd == 0 && x.Ps == 1 && x.Pt == 0 && x.Status==1 && _dbContext.SystemUserRoleMapping.Any(y => y.SystemUserUuid == AuthContextService.CurrentUser.Guid && y.SystemRoleUuid == x.SystemRoleUuid)).Select(x=>new SystemMenu
                {
