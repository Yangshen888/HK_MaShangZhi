...
//using HaiKanStudentDiningManagementSystem.Api.Entities.QueryModels.SystemPermission;
...
using HaiKanStudentDiningManagementSystem.Api.RequestPayload.Rbac.Permission;
using HaiKanStudentDiningManagementSystem.Api.ViewModels.Rbac.SystemPermission;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using static HaiKanStudentDiningManagementSystem.Api.Entities.SystemUser;
using HaiKanStudentDiningManagementSystem.Api.Models.QueryModels;
...
namespace HaikanEnrollmentRegistration.Api.Controllers.Api.V1.Rbac
{
    ...
    public class PermissionController : Controller
    {
        ...
        public PermissionController(haikanSDMSContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult Edit(PermissionEditViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                if (entity.Name != model.Name && _dbContext.SystemPermission.Any(x => x.Name == model.Name && x.SystemMenuUuid == model.SystemMenuUuid))
                {
                    response.SetFailed("权限名称已存在");
                    return Ok(response);
                }
                if (entity.ActionCode != model.ActionCode && _dbContext.SystemPermission.Any(x => x.ActionCode == model.ActionCode && x.SystemPermissionUuid != model.SystemPermissionUuid))
                {
                    response.SetFailed("权限操作码已存在");
                    return Ok(response);
                }
...
WHERE P.IsDeleted=0 AND P.Status=1";
                ...
                else
                {
                    ...
                    {
                        //tree = tree.FindAll(x => x.Title != "系统管理");
                        var roleEntity = _dbContext.SystemRole.FirstOrDefault(x => x.SchoolUuid == AuthContextService.CurrentUser.SchoolGuid && x.IsCreEdu == 1);
                        if (roleEntity != null)
                        {
                            var query = from srpm in _dbContext.SystemRolePermissionMapping
                                        join sp in _dbContext.SystemPermission
                                        on srpm.SystemPermissionUuid equals sp.SystemPermissionUuid
                                        join sm in _dbContext.SystemMenu
                                        on sp.SystemMenuUuid equals sm.SystemMenuUuid
                                        where srpm.SystemRoleUuid == roleEntity.SystemRoleUuid
                                        select new
                                        {
                                            sm.SystemMenuUuid,
                                        };
                            var query2 = from sm in _dbContext.SystemMenu
                                         join q in query
                                         on sm.SystemMenuUuid equals q.SystemMenuUuid into result
                                         from item in result.DefaultIfEmpty()
                                         where sm.ParentGuid == Guid.Empty && sm.IsDeleted == 0 && sm.Status == 1 && item.SystemMenuUuid == null
                                         select sm.SystemMenuUuid;
                            var list = query2.ToList();
                            menu = menu.FindAll(x => (!list.Contains(x.Guid))&& (!list.Contains(x.ParentGuid.Value)));
                            var suid= menu.Find(x => x.Title == "系统管理").Guid;
                            menu = menu.FindAll(x => x.Guid != suid && x.ParentGuid != suid);
                            var query3 = _dbContext.SystemRolePermissionMapping.Where(x => x.SystemRoleUuid == Guid.Parse(code));
                            var query4 = from sp in _dbContext.SystemPermission
                                         join sm in query3
